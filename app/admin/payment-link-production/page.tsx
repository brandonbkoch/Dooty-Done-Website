...
export default function PaymentLinkProductionPage() {
  const router = useRouter()

  const [customers, setCustomers] = useState<Customer[]>([])
  const [selectedCustomerId, setSelectedCustomerId] = useState("")
  const [amount, setAmount] = useState("1.00")
  const [name, setName] = useState("Dooty Done Production Smoke Test")
  const [description, setDescription] = useState(
    "Production payment processing smoke test for Dooty Done."
  )
  const [paymentNote, setPaymentNote] = useState(
    "Dooty Done production smoke test — refund immediately after verification."
  )
  ...
  const handleCreatePaymentLink = async () => {
    ...
    const confirmed = window.confirm(
      `Create a $${numericAmount.toFixed(
        2
      )} Square Production payment link for ${
        selectedCustomer
          ? `${selectedCustomer.first_name} ${selectedCustomer.last_name}`
          : "a customer"
      }?\n\nThis creates a Production checkout link. Creating the link does not charge anyone yet.`
    )
    ...
    try {
      ...
      const response = await fetch(
        "/api/square-create-payment-link-production",
        {
          method: "POST",
          headers: {
      ...
      setSuccessMessage(
        "Success! Your Square Production payment link was created."
      )
    } catch (error) {
      console.error(
      ...
      <div className="mx-auto max-w-2xl">
        <div className="rounded-3xl border border-[#0A1821]/10 bg-white p-6 shadow-sm sm:p-8">
          ...
          <p className="mt-3 text-sm leading-6 text-[#0A1821]/60">
            This temporary page creates a Square Production
